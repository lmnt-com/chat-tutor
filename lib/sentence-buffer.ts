export class SentenceBuffer {
  ...
  addText(text: string): void {
    ...
    if (
      this.buffer.length >= this.processingThreshold ||
      this.hasPotentialSentenceEnding(text)
    ) {
      this.processBuffer();
    }
  }
  ...
  private processBuffer(): void {
    ...
    while ((match = sentencePattern.exec(this.buffer)) !== null) {
      ...
      const sentenceText = this.buffer
        .substring(lastIndex, punctuationEnd)
        .trim();
      ...
      if (sentenceText) {
        ...
        const endPos =
          this.totalCharacters - this.buffer.length + punctuationEnd;
