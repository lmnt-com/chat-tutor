...
export function HighlightedMessage({
  ...
  const renderParts = () => {
    ...
    sentencesArray.forEach((sentence, index) => {
      ...
      if (startPos > lastPosition) {
        ...
        if (beforeText) {
          parts.push(<span key={`before-${index}`}>{beforeText}</span>);
        }
      }
        ...
        parts.push(
          <span
            ...
            className={cn(
              ...
              isHighlighted &&
                "border-b-2 border-yellow-400 bg-yellow-100 dark:bg-yellow-800/30",
            )}
          >
            {actualSentenceText}
          </span>,
  ...
  return <p className={cn(textSize, className)}>{renderParts()}</p>;
}
