...
export class Thread {
    ...
    async save(
        ...
        try {
            if (this.threadId) {
                ...
                const title = assistantResponse.slice(0, THREAD_TITLE_MAX_LENGTH) || "New Chat"
    ...
    private async updateExisting(threadId: string, messages: Message[], assistantResponse: string) {
        ...
        const { error: messagesError } = await this.supabase.from("chat_messages").insert(
